...
const TestGlobe = () => {
    ...
    try {
      ...
      if (response.ok && data.length > 0) {
        ...
        for (let i = 0; i < data.length; i++) {
          const coordinates = data[i]?.latlng;
  ...
  // Example usage
  getCountryCoordinates().then((coordinatesArray) => {
    if (coordinatesArray) {
      console.log("Coordinates Array:", coordinatesArray);
    }
  });
  ...
  useEffect(() => {  
    // Example: Add a dot at latitude 40, longitude -74 (New York)
    const latitude = 39.0; // in degrees
    const longitude = 35.0; // in degrees
  ...
  for (let i = 0; i < arr.length; i++) {
    ...
    const y = Math.cos(phi);
    const z = Math.sin(phi) * Math.sin(theta);
     const dotMesh = new THREE.Mesh(dotGeometry, new THREE.MeshBasicMaterial({ color: "blue" }));
